...
namespace Qs.Runtime
{
    ...
    public class QsMarshal
    {
            ...
            foreach (var p in parameters)
            {
                ...
                else if (p.QsNativeValue is QsScalar)
                {
                    ...
                    if(paramInfos[iy].ParameterType.IsSubclassOf(typeof(QsValue)))
                    {
                        ...
                        object nativeValue = Convert.ChangeType(scalar, paramInfos[iy].ParameterType, null);
                    ...
                    else if(paramInfos[iy].ParameterType.IsGenericType)
                    {
                        ...
                        {
                            object nativeValue = Convert.ChangeType(scalar.NumericalQuantity, paramInfos[iy].ParameterType, null);
                            NativeParameters.Add(nativeValue);
                        }
                    }
                    ...
                    {
                        object nativeValue = Convert.ChangeType(scalar.NumericalQuantity.Value, paramInfos[iy].ParameterType, null);
                        NativeParameters.Add(nativeValue);
                    }
                }
                ...
                else if (p.QsNativeValue is QsVector)
                {
                    ...
                    if (paramInfos[iy].ParameterType.IsArray)
                    {
                        ...
                        Type ArrayType = Type.GetType(paramInfos[iy].ParameterType.FullName.Trim('[', ']'));
                        Array arr = Array.CreateInstance(ArrayType, vec.Count);
                        for (int i = 0; i < vec.Count; i++)
                        {
                            object val = Convert.ChangeType(vec[i].NumericalQuantity.Value, ArrayType, null);
                            arr.SetValue(val, i);
                        }
        ...
        public static QsValue NativeToQsConvert(object value)
        {
            ...
            if (vType == typeof(int) || vType == typeof(double) || vType == typeof(float) || vType == typeof(float) || vType == typeof(long) || vType == typeof(uint) || vType == typeof(ushort) || vType == typeof(ulong))
            {
                double vv = (double)Convert.ChangeType(value, typeof(double), null);
                return new QsScalar
                {
            ...
            if (vType == typeof(int[]) || vType == typeof(double[]) || vType == typeof(float[]) || vType == typeof(float[]) || vType == typeof(long[]) || vType == typeof(uint[]) || vType == typeof(ushort[]) || vType == typeof(ulong[]))
            {
                ...
                foreach (var m in rr)
                {
                    var r = (double)Convert.ChangeType(m, typeof(double), null);
        ...
        public static Expression QsToNativeConvert(Type targetType, Expression value)
        {
            ...
            if (targetType == typeof(int) || targetType == typeof(double) || targetType == typeof(float) || targetType == typeof(float) 
                || targetType == typeof(long) || targetType == typeof(uint) || targetType == typeof(ushort) || targetType == typeof(ulong))
            {
                var nq = Expression.Property(Expression.Convert(value, typeof(QsScalar)), "NumericalQuantity");
            ...
            if (targetType == typeof(int[]) || targetType == typeof(double[]) || targetType == typeof(float[]) || targetType == typeof(float[]) || targetType == typeof(long[]) || targetType == typeof(uint[]) || targetType == typeof(ushort[]) || targetType == typeof(ulong[]))
            {
                // then the value should be a qsvector or qsmatrix
                
                var mi = typeof(QsMarshal).GetMethod("ToNumericArray", BindingFlags.Static | BindingFlags.NonPublic).MakeGenericMethod(targetType.GetElementType());
            ...
            if (value is QsVector)
            {
                ...
                while (ix >= 0)
                {
                    ...
                    if (targetType == typeof(long))
                        dd[ix] = (Numeric)(object)(long)ListStorage[ix].NumericalQuantity.Value;
                    if (targetType == typeof(uint))
                        dd[ix] = (Numeric)(object)(uint)ListStorage[ix].NumericalQuantity.Value;
                    if (targetType == typeof(ushort))
                        dd[ix] = (Numeric)(object)(ushort)ListStorage[ix].NumericalQuantity.Value;
                    if (targetType == typeof(ulong))
                        dd[ix] = (Numeric)(object)(ulong)ListStorage[ix].NumericalQuantity.Value;
